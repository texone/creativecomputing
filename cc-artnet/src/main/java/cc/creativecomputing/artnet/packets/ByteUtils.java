...
public class ByteUtils {
    ...
    public static boolean compareBytes(ByteBuffer theData, byte[] theCompareData, int theLength){
    	if(theData.remaining() < theLength)return false;
    	for (int i = 0; i < theLength; i++) {
    		if(theData.get() != theCompareData[i])return false;
        }
        return true;
    }
    
    public static boolean compareBytes(ByteBuffer theData, byte[] theCompareData){
    	return compareBytes(theData, theCompareData, theCompareData.length);
    }
     ...
     * @return
     */
    public final static int getInt16LE(ByteBuffer theData) {
    	byte a = theData.get();
    	byte b = theData.get();
        return (byteToUint(b) << 8) | byteToUint(a);
    }
