...
public enum CCArtNetOpCode {
	 ...
	 */
	POLL(0x2000, ArtPollPacket.class), 
	/**
	 * This is an ArtPollReply Packet. It contains device status information.
	 */
	POLL_REPLY(0x2100, ArtPollReplyPacket.class), 
	/**
	 ...
	 */
	OUTPUT(0x5000, ArtDmxPacket.class), 
	/**
	...
	public final int id;
	private final Class<? extends ArtNetPacket> _myPacketClass;

	private CCArtNetOpCode(int theID, Class<? extends ArtNetPacket> theClass) {
		id = theID;
		_myPacketClass = theClass;
	}

	public ArtNetPacket createPacket() {
		if(_myPacketClass == null)return null;
		
		try {
			return _myPacketClass.newInstance();
	...
	public static CCArtNetOpCode fromID(int theID){
		for (CCArtNetOpCode myOpCode : CCArtNetOpCode.values()) {
			if (theID == myOpCode.id) {
				return myOpCode;
			}
		}
		return null;
	...
	public static ArtNetPacket createPacket(int theID){
		CCArtNetOpCode myCode = fromID(theID);
		if(myCode == null)return null;
		
		ArtNetPacket myPacket = myCode.createPacket();
		
		if(myPacket == null){
			CCLog.info("packet type valid, but not yet supported: " + myCode);
			return null;
		}
