...
public class CCFlowFieldFollowDraw extends CCApp{
...
//			myResult.normalize();
	...
	public void setup(){
		...
		for(int i = 0; i < 400;i++){
			...
			myParticle.velocity(CCVecMath.random3f(100));
			myParticle.position = CCVecMath.random3f(10);
			myParticle.maxSpeed = 4.5F;
			myParticle.maxForce = 5.3F;
			
			_mySimulation.addParticle(myParticle);
			_myParticles.add(myParticle);
		}
		
		_myMesh = new CCMesh(CCDrawMode.QUADS,400 * 4);
...
//		g.endShape();
	...
	public static void main(String[] args) {
		CCApplicationManager myManager = new CCApplicationManager(CCFlowFieldFollowDraw.class);
