 ...
 */
package cc.creativecomputing.demo.simulation;

import cc.creativecomputing.CCApp;
import cc.creativecomputing.CCApplicationManager;
import cc.creativecomputing.control.CCControl;
import cc.creativecomputing.control.CCControlUI;
import cc.creativecomputing.demo.simulation.particles.CCTestParticle;
...
public class CCSimulationThreadTest extends CCApp{
...
//			myResult.normalize();
	...
	private CCParticleGroup<CCTestParticle> _myThreadedParticleGroup = new CCParticleGroup<CCTestParticle>();
...
//		_myParticleGroup.addForce(myFlowFieldFollow);
//		
...
//			
//			_myParticleGroup.addParticle(myParticle);
//		}
//		
//		_mySimulation.addParticleGroup(_myParticleGroup);
		
		_myThreadedParticleGroup.addForce(myFlowFieldFollow);
		...
		for(int i = 0; i < 8000;i++){
			...
			_myThreadedParticleGroup.addParticle(myParticle);
		}
		
		_mySimulation.addParticleGroup(_myThreadedParticleGroup,0);
		
		_myMesh = new CCMesh(CCDrawMode.QUADS,8000 * 4);
...
//		for(CCParticle myParticle:_myParticleGroup.particles()){
//			myParticle.maxSpeed = Settings.maxSpeed;
//			myParticle.maxForce = Settings.maxForce;
//		}
		for(CCParticle myParticle:_myThreadedParticleGroup.particles()){
			myParticle.maxSpeed = Settings.maxSpeed;
			myParticle.maxForce = Settings.maxForce;
		}
	...
	public void draw(){
		...
		_myArcball.draw(g);
		g.color(255);
		g.blendMode(CCGraphics.ADD);
		g.frustum().update();
		_myMesh.clearVertices();
//		for(CCTestParticle myParticle:_myParticleGroup.particles()){
//			myParticle.frustumWrap(g);
//			myParticle.draw(_myMesh);
//		}
		for(CCTestParticle myParticle:_myThreadedParticleGroup.particles()){
			myParticle.frustumWrap(g);
			myParticle.draw(_myMesh);
		}
		_myMesh.draw(g);
	...
	public static void main(String[] args) {
		CCApplicationManager myManager = new CCApplicationManager(CCSimulationThreadTest.class);
