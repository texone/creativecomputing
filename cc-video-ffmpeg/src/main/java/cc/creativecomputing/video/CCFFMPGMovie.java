...
public class CCFFMPGMovie extends CCMovieData {
	...
	private AVCodecContext _myCodecContext;
	...
	public CCFFMPGMovie(final CCAnimator theAnimator, final Path thePath) {
		...
		_myCodecContext = avcodec_alloc_context3(null);
		avcodec_parameters_to_context(_myCodecContext, _myVideoStream.codecpar());
	...
	private void checkFrame() {
		...
		if (_myPacket.stream_index() == _myVideoStreamIndex) {
			...
			if (_myPacket.dts() != AV_NOPTS_VALUE) {
				_myPTS = _myFrame.best_effort_timestamp();
			} else {
				_myPTS = 0;
			}


			_myPTS *= av_q2d(_myVideoStream.time_base());
