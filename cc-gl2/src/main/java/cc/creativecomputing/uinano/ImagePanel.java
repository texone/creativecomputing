package cc.creativecomputing.uinano;
import java.util.*;

/*
    src/imagepanel.cpp -- Image panel widget which shows a number of
    square-shaped icons

    NanoGUI was developed by Wenzel Jakob <wenzel.jakob@epfl.ch>.
    The widget drawing code is based on the NanoVG demo application
    by Mikko Mononen.

    All rights reserved. Use of this source code is governed by a
    BSD-style license that can be found in the LICENSE.txt file.
*/

/*
    nanogui/imagepanel.h -- Image panel widget which shows a number of
    square-shaped icons

    NanoGUI was developed by Wenzel Jakob <wenzel.jakob@epfl.ch>.
    The widget drawing code is based on the NanoVG demo application
    by Mikko Mononen.

    All rights reserved. Use of this source code is governed by a
    BSD-style license that can be found in the LICENSE.txt file.
*/
/** \file */


{

/**
 * \class ImagePanel imagepanel.h nanogui/imagepanel.h
 *
 * \brief Image panel widget which shows a number of square-shaped icons.
 */
public class ImagePanel extends Widget
{
	public ImagePanel(Widget parent)
	{
		super(parent);
		this.mThumbSize = 64;
		this.mSpacing = 10;
		this.mMargin = 10;
		this.mMouseIndex = -1;
	}

	public final void setImages(ArrayList<tangible.Pair<Integer, String>> data)
	{
		mImages = data;
	}
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: const ClassicVector<tangible.Pair<int, String>>& images() const
	public final ArrayList<tangible.Pair<Integer, String>> images()
	{
		return mImages;
	}

//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: System.Action<int> callback() const
	public final tangible.Action1Param<Integer> callback()
	{
		return mCallback;
	}
	public final void setCallback(tangible.Action1Param<Integer> callback)
	{
		mCallback = callback;
	}

	@Override
	public boolean mouseMotionEvent(Vector2i p, Vector2i rel, int button, int modifiers)
	{
		mMouseIndex = indexForPosition(p);
		return true;
	}
	@Override
	public boolean mouseButtonEvent(Vector2i p, int button, boolean down, int modifiers)
	{
		int index = indexForPosition(p);
		if (index >= 0 && mCallback && down)
		{
			mCallback(index);
		}
		return true;
	}
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: virtual Vector2i preferredSize(NVGcontext * ctx) const override
	@Override
	public Vector2i preferredSize(NVGcontext ctx)
	{
		Vector2i grid = gridSize();
		return Vector2i(grid.x() * mThumbSize + (grid.x() - 1) * mSpacing + 2 * mMargin, grid.y() * mThumbSize + (grid.y() - 1) * mSpacing + 2 * mMargin);
	}
	@Override
	public void draw(NVGcontext ctx)
	{
		Vector2i grid = gridSize();

		for (size_t i = 0; i < mImages.size(); ++i)
		{
			Vector2i p = mPos + Vector2i.Constant(mMargin) + Vector2i((int) i % grid.x(), (int) i / grid.x()) * (mThumbSize + mSpacing);
			int imgw;
			int imgh;

			nvgImageSize(ctx, mImages[i].first, imgw, imgh);
			float iw;
			float ih;
			float ix;
			float iy;
			if (imgw < imgh)
			{
				iw = mThumbSize;
				ih = iw * (float)imgh / (float)imgw;
				ix = 0F;
				iy = -(ih - mThumbSize) * 0.5f;
			}
			else
			{
				ih = mThumbSize;
				iw = ih * (float)imgw / (float)imgh;
				ix = -(iw - mThumbSize) * 0.5f;
				iy = 0F;
			}

			NVGpaint imgPaint = nvgImagePattern(ctx, p.x() + ix, p.y() + iy, iw, ih, 0, mImages[i].first, mMouseIndex == (int)i != null ? 1.0 : 0.7);

			nvgBeginPath(ctx);
			nvgRoundedRect(ctx, p.x(), p.y(), mThumbSize, mThumbSize, 5);
			nvgFillPaint(ctx, imgPaint);
			nvgFill(ctx);

			NVGpaint shadowPaint = nvgBoxGradient(ctx, p.x() - 1, p.y(), mThumbSize + 2, mThumbSize + 2, 5, 3, nvgRGBA(0, 0, 0, 128), nvgRGBA(0, 0, 0, 0));
			nvgBeginPath(ctx);
			nvgRect(ctx, p.x() - 5,p.y() - 5, mThumbSize+10,mThumbSize+10);
			nvgRoundedRect(ctx, p.x(),p.y(), mThumbSize,mThumbSize, 6);
			nvgPathWinding(ctx, NVG_HOLE);
			nvgFillPaint(ctx, shadowPaint);
			nvgFill(ctx);

			nvgBeginPath(ctx);
			nvgRoundedRect(ctx, p.x() + 0.5f,p.y() + 0.5f, mThumbSize-1,mThumbSize-1, 4 - 0.5f);
			nvgStrokeWidth(ctx, 1.0f);
			nvgStrokeColor(ctx, nvgRGBA(255,255,255,80));
			nvgStroke(ctx);
		}
	}
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: Vector2i gridSize() const
	protected final Vector2i gridSize()
	{
		int nCols = 1 + Math.max(0, (int)((mSize.x() - 2 * mMargin - mThumbSize) / (float)(mThumbSize + mSpacing)));
		int nRows = ((int) mImages.size() + nCols - 1) / nCols;
		return Vector2i(nCols, nRows);
	}
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: int indexForPosition(const Vector2i &p) const
	protected final int indexForPosition(Vector2i p)
	{
		Vector2f pp = ((p - mPos).<Float>cast() - Vector2f.Constant(mMargin)) / (float)(mThumbSize + mSpacing);
		float iconRegion = mThumbSize / (float)(mThumbSize + mSpacing);
		boolean overImage = pp.x() - Math.floor(pp.x()) < iconRegion && pp.y() - Math.floor(pp.y()) < iconRegion;
		Vector2i gridPos = pp.<Integer>cast();
		Vector2i grid = gridSize();
		overImage &= ((gridPos.array() >= 0).all() && (gridPos.array() < grid.array()).all());
		return overImage ? (gridPos.x() + gridPos.y() * grid.x()) : -1;
	}
	protected ArrayList<tangible.Pair<Integer, String>> mImages = new ArrayList<tangible.Pair<Integer, String>>();
	protected tangible.Action1Param<Integer> mCallback;
	protected int mThumbSize;
	protected int mSpacing;
	protected int mMargin;
	protected int mMouseIndex;
//C++ TO JAVA CONVERTER TODO TASK: The following statement was not recognized, possibly due to an unrecognized macro:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW
}