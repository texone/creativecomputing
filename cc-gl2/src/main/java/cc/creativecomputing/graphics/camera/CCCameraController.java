...
//			feed();
	...
	private final CCGLWindow _myApp;
	...
	public CCCameraController(final CCGLWindow theApp, final CCGraphics theG, final double theDistance) {
		this(theApp, theG, 0,0,1,1,0, 0, 0, theDistance);
	}

	public CCCameraController(
		final CCGLWindow theApp, 
		...
		_myApp.updateEvents.add(theTimer -> {
	...
	public void setActive(final boolean theIsActive) {
		...
		if (_myIsActive) {
			_myApp.mousePressEvents.add(mousePressed);
			_myApp.mouseReleaseEvents.add(mouseReleased);
			_myApp.scrollEvents.add(_myWheelHandler);
			_myApp.mouseDragEvents.add(mouseDragged);
			_myApp.keyReleaseEvents.add(_myKeyListener);
		} else {
			_myApp.mousePressEvents.remove(mousePressed);
			_myApp.mouseReleaseEvents.remove(mouseReleased);
			_myApp.scrollEvents.remove(_myWheelHandler);
			_myApp.mouseDragEvents.remove(mouseDragged);
			_myApp.keyReleaseEvents.remove(_myKeyListener);
		}
	}
...
//		feed();
	...
	abstract public class AbstractInterp{
		...
		void cancel() {
			_myApp.updateEvents.remove(this::update);
		}
