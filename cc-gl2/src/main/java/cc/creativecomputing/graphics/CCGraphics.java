...
import static org.lwjgl.opengl.GL12.GL_SINGLE_COLOR;
import static org.lwjgl.opengl.GL13.GL_COMBINE;
import static org.lwjgl.opengl.GL13.GL_NORMAL_MAP;
import static org.lwjgl.opengl.GL13.GL_REFLECTION_MAP;
import static org.lwjgl.opengl.GL13.GL_TEXTURE0;
import static org.lwjgl.opengl.GL13.glActiveTexture;
import static org.lwjgl.opengl.GL13.glMultiTexCoord1d;
import static org.lwjgl.opengl.GL13.glMultiTexCoord2d;
import static org.lwjgl.opengl.GL13.glMultiTexCoord3d;
import static org.lwjgl.opengl.GL13.glMultiTexCoord4d;
...
import static org.lwjgl.opengl.GL14.GL_POINT_SIZE_MAX;
import static org.lwjgl.opengl.GL14.GL_POINT_SIZE_MIN;
 ...
 * OPEN OpenGL is strictly defined as "a software interface to 
 ...
 * OpenGL. It is aimed to simplify the access to OpenGL.
 * 
...
public class CCGraphics{
	...
	public CCGraphics( int theWidth, int theHeight){
...
//		lightModelTwoSide(true);
	...
	public int getInteger(int theGLIID) {
		glGetIntegerv(theGLIID, _myIntegerGet);
	...
	public IntBuffer getIntBuffer(int theGLID, int theNumberOfValues) {
		...
		glGetIntegerv(theGLID, myResult);
		myResult.rewind();
		return myResult;
	}
	...
	public float getFloat(int theGLIID) {
		glGetFloatv(theGLIID, _myFloatGet);
	...
	public float[] getFloatArray(int theGLID, int theNumberOfValues) {
		...
		glGetFloatv(theGLID, result);
		return result;
	}

		public String getString(int theGLID) {
			return glGetString(theGLID);
		}
		...
		public String[] extensions() {
			return getString(GL_EXTENSIONS).split(" ");
		}
	...
	public void resize(int theWidth, int theHeight){
		_myWidth = theWidth;
		_myHeight = theHeight;
		_myCamera.set(theWidth, theHeight);
	}
	...
	public void scissor(final double theX, final double theY, final double theWidth, final double theHeight) {
		glEnable(GL_SCISSOR_TEST);
		glScissor((int)theX, (int)theY, (int)theWidth, (int)theHeight);
	}
	...
	public void blendMode(final CCBlendFactor theSrcFactor, final CCBlendFactor theDstFactor) {
		...
		glBlendFunc(theSrcFactor.glId, theDstFactor.glId);
	...
	public void blendMode(
		...
		glBlendFunc(theSrcFactor.glId, theDstFactor.glId);
		glBlendEquation(theEquation.glId);
	}
	...
	public void blendMode(
		...
		glBlendFuncSeparate(theSrcFactor.glId, theDstFactor.glId, theSrcAlphaFactor.glId, theDstAlphaFactor.glId);
		glBlendEquationSeparate(theColorEquation.glId, theAlphaEquation.glId);
	}
...
// blend mode keyword definitions
	...
	public int get(int x, int y){
		glReadPixels(x, y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, getsetBuffer);
	...
	public void clearColor(final double theRed, final double theGreen, final double theBlue, final double theAlpha) {
		glClearColor((float)theRed, (float)theGreen, (float)theBlue, (float)theAlpha);
	}
	...
	public void texture(final CCTexture theTexture) {
		...
		glEnable(_myTextures[0].target().glID);
		_myTextures[0].bind();
		_myDrawTexture = true;
	...
	public void texture(final int theTextureUnit, final CCTexture theTexture) {
		...
		glEnable(_myTextures[theTextureUnit].target().glID);
	...
	public void noTexture() {
		for (int i = 0; i < _myTextures.length; i++) {
			if (_myTextures[i] != null) {
				...
				glDisable(_myTextures[i].target().glID);
				_myTextures[i] = null;
	...
	public void enableTextures() {
		for (int i = 0; i < _myTextures.length; i++) {
			if (_myTextures[i] != null) {
				glActiveTexture(GL_TEXTURE0 + i);
				glEnable(_myTextures[i].target().glID);
			}
		}
	}
	...
	public void color(double theGray, final double theAlpha) {
		glColor4d(theGray,theGray,theGray,theAlpha);
	}
	
	public void color(double theGray) {
		glColor4d(theGray,theGray,theGray,1);
	}
	...
	public void color(final int theRed, final int theGreen, final int theBlue){
		color(theRed / 255d, theGreen / 255d, theBlue / 255d);
	}
	
	public void color(final int theRed, final int theGreen, final int theBlue, final int theAlpha){
		color(theRed / 255d, theGreen / 255d, theBlue / 255d, theAlpha / 255d);
	}
	
	public CCColor color(){
		double[] myColor = new double[4];
		glGetDoublev(GL_CURRENT_COLOR,myColor);
	...
	public void matrixMode(final CCMatrixMode theMode){
		glMatrixMode(theMode.glID);
	}
	
	public CCMatrixMode matrixMode() {
		switch(getInteger(GL_MATRIX_MODE)) {
		case GL_TEXTURE:
	...
	public void loadMatrix(final CCMatrix4x4 theMatrix){
		try (MemoryStack stack = stackPush()) {
			glLoadMatrixd(theMatrix.toDoubleBuffer(stack.mallocDouble(16)));
		}
	}
	...
	public void applyMatrix(final CCMatrix4x4 theMatrix){
		try (MemoryStack stack = stackPush()) {
			glMultMatrixd(theMatrix.toDoubleBuffer(stack.mallocDouble(16)));
		}
	}
	
	public void applyTransform(final CCTransform theTransform){
		try (MemoryStack stack = stackPush()) {
			glMultMatrixd(theTransform.getGLApplyMatrix(stack.mallocDouble(16)));
		}
	}
	...
	public void applyMatrix(
		...
		try (MemoryStack stack = stackPush()) {
			final DoubleBuffer myMatrixBuffer = stack.mallocDouble(16);
	...
	public void rotate(final double theAngle, final double theX, final double theY, final double theZ) {
		glRotated(theAngle, theX, theY, theZ);
	}
	...
	public void scale(final double theX, final double theY) {
		glScaled(theX, theY, 1);
	}

	public void scale(final double theSize) {
		glScaled(theSize, theSize, theSize);
	}
	...
	public void vertex(final CCVector3 theVector){
		glVertex3d(theVector.x, theVector.y, theVector.z);
	}
	...
	public void vertex(final CCVector2 theVertex,final CCVector2 theTextureCoords){
		...
		glVertex2d(theVertex.x, theVertex.y);
	}
	...
	public void vertex(final CCVector3 theVertex, final CCVector2 theTextureCoords){
		textureCoords2D(theTextureCoords);
		glVertex3d(theVertex.x, theVertex.y, theVertex.z);
	}
	...
	private void bezier(final DoubleBuffer thePointBuffer, final int theNumberOfPoints){
		glMap1d(GL_MAP1_VERTEX_3,0.0f,bezierDetail,3,theNumberOfPoints,thePointBuffer);
		glEnable(GL_MAP1_VERTEX_3);
		glMapGrid1d(bezierDetail,0,bezierDetail);
		glEvalMesh1(GL_LINE,0,bezierDetail);
	}
	...
	public void bezier(
		...
		try (MemoryStack stack = stackPush()) {
			final DoubleBuffer myPoints = stack.mallocDouble(12);
	...
	public void bezier(
		...
		try (MemoryStack stack = stackPush()) {
			final DoubleBuffer floatbuffer = stack.mallocDouble(12);
	...
	public void endBezier(){
		glMap1d(GL_MAP1_VERTEX_3,0.0f,bezierDetail,3,31,DoubleBuffer.wrap(_myBezierCoords));
		glEnable(GL_MAP1_VERTEX_3);
		glMapGrid1d(bezierDetail,0,bezierDetail);
		glEvalMesh1(GL_LINE,0,bezierDetail);
	}
	...
	public double lineWidthMaximum(){
		return getFloatArray(GL_LINE_WIDTH_RANGE, 2)[1];
	}
	...
	public double lineWidthMinimum(){
		return getFloatArray(GL_LINE_WIDTH_RANGE, 2)[0];
	}
	...
	public double lineWidthGranularity(){
		return getFloat(GL_LINE_WIDTH_GRANULARITY);
	}
	...
	public void lineStipple(
		...
		glLineStipple(theFactor,Short.parseShort(thePattern, 2));
	}
	...
	public void pointSize(final double thePointSize){
		glPointSize((float)thePointSize);
	}
	...
	public double pointSizeMaximum(){
		return getFloat(GL_POINT_SIZE_MAX);
	}

	public void pointSizeMaximum(final double thePointSize){
		glPointParameterf(GL_POINT_SIZE_MAX, (float)thePointSize);
	}
	...
	public double pointSizeMinimum(){
		return getFloat(GL_POINT_SIZE_MIN);
	}

	public void pointSizeMinimum(final double thePointSize){
		glPointParameterf(GL_POINT_SIZE_MIN, (float)thePointSize);
	}
	
	public float[] pointSizeRange(){
		return getFloatArray(GL_POINT_SIZE_RANGE, 2);
	}
	...
	public double pointSizeGranularity(){
		return getFloat(GL_POINT_SIZE_GRANULARITY);
	}
	...
	public void textureEnvironment(final CCTextureEnvironmentMode theMode){
		glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, theMode.glID);
	}
	...
	public enum CCTextureGenMode{
		 ...
		 */
		OBJECT_LINEAR(GL_OBJECT_LINEAR, GL_OBJECT_PLANE),
		 ...
		 * </pre>
		 */
		EYE_LINEAR(GL_EYE_LINEAR, GL_EYE_PLANE),
		 ...
		 * less overhead.
		 */
		SPHERE_MAP(GL_SPHERE_MAP,-1),
		NORMAL_MAP(GL_NORMAL_MAP,-1),
		REFLECTION_MAP(GL_REFLECTION_MAP,-1);
	...
	public enum CCTextureGenCoord{
		S(GL_S, GL_TEXTURE_GEN_S),
		T(GL_T, GL_TEXTURE_GEN_T),
		R(GL_R, GL_TEXTURE_GEN_R),
		Q(GL_Q, GL_TEXTURE_GEN_Q);
...
//		texGen(CCTextureGenCoord.Q, theMode);
	...
	public void texGen(final CCTextureGenCoord theCoord, final CCTextureGenMode theMode, double...theData) {
		glEnable(theCoord.glGenID);
		glTexGeni(theCoord.glID, GL_TEXTURE_GEN_MODE, theMode.glID);
		
		if(theMode.glPlaneId < 0 || theData == null || theData.length < 4)return;
		
		glTexGendv(theCoord.glID, theMode.glPlaneId, theData);
	}
	
	public void noTexGen() {
		glDisable(CCTextureGenCoord.S.glGenID);
		glDisable(CCTextureGenCoord.T.glGenID);
		glDisable(CCTextureGenCoord.R.glGenID);
		glDisable(CCTextureGenCoord.Q.glGenID);
	}
...
//			}
	...
	public void ortho(
		...
		glLoadIdentity();
		glOrtho(theLeft , theRight,  theBottom, theTop, theNear, theFar);
	...
	public void colorMaterial(final CCCullFace theFace, final CCColorMaterialMode theMode){
		...
		case EMISSION:
			...
			glColorMaterial(_myColorMaterialFace.glID, GL_EMISSION);
			break;
		case AMBIENT:
			glEnable(GL_COLOR_MATERIAL);
			glColorMaterial(_myColorMaterialFace.glID, GL_AMBIENT);
			break;
		case DIFFUSE:
			glEnable(GL_COLOR_MATERIAL);
			glColorMaterial(_myColorMaterialFace.glID, GL_DIFFUSE);
			break;
		case AMBIENT_AND_DIFFUSE:
			glEnable(GL_COLOR_MATERIAL);
			glColorMaterial(_myColorMaterialFace.glID, GL_AMBIENT_AND_DIFFUSE);
			break;
		case SPECULAR:
			glEnable(GL_COLOR_MATERIAL);
			glColorMaterial(_myColorMaterialFace.glID, GL_SPECULAR);
			break;
		}
	}
	...
	public void lightModelAmbient(final double theRed, final double theGreen, final double theBlue){
		...
		theColor[3] = 1.0f;
		glLightModelfv(GL_LIGHT_MODEL_AMBIENT, theColor);
	}
	...
	public void lightModelAmbient(final int theRed, final int theGreen, final int theBlue){
		...
		theColor[2] = theBlue;
		theColor[3] = 255;
		glLightModeliv(GL_LIGHT_MODEL_AMBIENT, theColor);
	}
	...
	public void lightModelTwoSide(final boolean theIsTwoSide){
		if(theIsTwoSide)glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, 1);
	...
	public void lightModelLocalViewer(final boolean theIsLocalViewer){
		if(theIsLocalViewer)glLightModeli(GL_LIGHT_MODEL_LOCAL_VIEWER, 1);
	...
	public void lightModelColorControl(final boolean theIsSingleColor){
		if(theIsSingleColor)glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL, GL_SINGLE_COLOR);
		else glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL, GL_SEPARATE_SPECULAR_COLOR);
	}
...
//	private CCText _myText = new CCText(CCFontIO.createTextureMapFont("arial", 10));
//	
...
//		if(_myText == null)_myText = new CCText(CCFontIO.createTextureMapFont("arial", 10));
...
//
	...
	public void copy(
		...
		glWindowPos2i(width()/2+dx1, height()/2 + dy1);
		glPixelZoom((float) dx2 / sx2, (float) dy2 / sy2);
		glCopyPixels(width()/2+sx1, height()/2 + sy1, sx2, sy2, GL_COLOR);
		glPixelZoom(1, 1);
	}
	...
	public void copy(
		...
		glCopyTexSubImage2D(des.target().glID, 0, dx1, dy1, sx1, sy1, sx2, sy2);
	}
	...
	public void updatePixels() {
		...
		glRasterPos2d(EPSILON, height() - EPSILON);
	...
	public void set(int x, int y, int argb){
		...
		glRasterPos2d(x + EPSILON, y + EPSILON);
		glDrawPixels(1, 1, GL_RGBA, GL_UNSIGNED_BYTE, getsetBuffer);
	}
