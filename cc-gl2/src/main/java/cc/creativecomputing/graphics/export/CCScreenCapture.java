...
public class CCScreenCapture {
	...
	static class PixelStorageModes {
		...
		void save() {
			packAlignment = glGetInteger(GL_PACK_ALIGNMENT, tmp);
			packRowLength = glGetInteger(GL_PACK_ROW_LENGTH, tmp);
			packSkipRows = glGetInteger(GL_PACK_SKIP_ROWS, tmp);
			packSkipPixels = glGetInteger(GL_PACK_SKIP_PIXELS, tmp);
			packSwapBytes = glGetInteger(GL_PACK_SWAP_BYTES, tmp);
	...
	public static void writeToTargaFile(
		...
		try{
			...
			int readbackType = (alpha ? GL_ABGR_EXT : GL_BGR);
	
			// read the BGR values into the image buffer
			glReadPixels(x, y, width, height, readbackType, GL_UNSIGNED_BYTE, bgr);
	 ...
	 *             OpenGL-related error occurred
	 */
	public static BufferedImage readToBufferedImage(int width, int height)  {
	...
	public static BufferedImage readToBufferedImage(int x, int y, int width, int height, boolean alpha){
		...
		glReadPixels(x, y, width, height, readbackType, GL_UNSIGNED_BYTE,ByteBuffer.wrap(((DataBufferByte) image.getRaster().getDataBuffer()).getData()));
	...
	private static int glGetInteger(int pname, int[] tmp) {
