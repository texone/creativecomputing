...
public class CCMultiFontText {
	...
	public static class CCTextPart{
		...
		public CCTextPart(CCFont<?> theFont, int theSize, CCColor theColor, String theText){
			_myText = theText;
			_myFont = theFont;
			_myTextSize = theSize;
		...
		public CCTextPart(CCFont<?> theFont, int theSize, String theText){
			this(theFont, theSize, null, theText);
		}
		...
		public double ascent() {
			return _myFont.ascent() * _myFont.scaleForPixelHeight(_myTextSize);
		}

		public double descent() {
			return _myFont.descent() * _myFont.scaleForPixelHeight(_myTextSize);
		}
	...
	public class CCTextGridLinePart{
		...
		private CCTextGridLinePart(
			...
			final int theIndex,
			final int theStart, final int theEnd, final String theTextBuffer,
			...
			for (int index = theStart; index < theEnd; index++) {
				...
				final char myChar = theTextBuffer.charAt(index);
				final int myIndex = _myTextPart.font().index(myChar);
					
				if(myTextAlign == CCTextAlign.JUSTIFY) {
					theX += mySpace;
				}else {
					theX += _myTextPart.font().kernAdvance(myLastIndex, myIndex) * _myTextPart.size();
				}
			...
			_myHasLineBreak = theTextBuffer.charAt(theEnd - 1) == '\n';
		}
		...
		public void drawChar(CCGraphics g, int theCharIndex) {
			_myTextPart.font().fontChar(_myFontCharIndices[theCharIndex]).drawVertices(
				g, _myTextPart.size(), 
	...
	public class CCTextGrid{
		...
		CCTextGridLinePart createTextPart(
			...
			final int theIndex,
			final int theStart, final int theStop, final String theTextBuffer,
			double theX, final double theY
		) {
			CCLog.info(theTextBuffer, theStart, theStop);
			String myText = theTextBuffer.substring( theStart, theStop - theStart);
	...
	public void addText(String theText, CCFont<?> theFont, int theSize){
		_myTextParts.add(new CCTextPart(theFont, theSize, null, theText));
	}
	
	public void addText(String theText, CCFont<?> theFont, int theSize, CCColor theColor){
		_myTextParts.add(new CCTextPart(theFont, theSize, theColor, theText));
	}
