...
public class GLTextureCubeMap extends GLTexture{
	 ...
	 */
	public GLTextureCubeMap(CCImage theImage) {
		super(GLTextureTarget.TEXTURE_CUBE_MAP);
		data(theImage);
	}
	
	private void compressedData(final CCImage theImage, final int theID) {
		GLPixelDataInternalFormat myInternalFormat = internalFormat(theImage);
	...
	public void dataImplementation(CCImage theImage) {
		if(theImage.mipmapData().length < 6) {
			throw new GLException("Cubemap requires 6 images to be generated. Make sure you pass a data object that contains 6 images.");
		}
