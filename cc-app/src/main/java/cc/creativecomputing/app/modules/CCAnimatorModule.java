 ...
 * 
 */
public class CCAnimatorModule extends CCAnimator implements CCAppModule<CCAnimatorListener>{
	 ...
	 **/
	public CCAnimatorModule(String theID) {
		super(CCAnimatorListener.class, theID);
	}
	
	public CCAnimatorModule(){
		this("animator");
	}
				...
				while (!_myShouldStop) {
					try {
						...
						if (_myListeners.size() == 0) {
							synchronized (CCAnimatorModule.this) {
								while (_myListeners.size() == 0 && !_myShouldStop) {
									try {
										CCAnimatorModule.this.wait();
