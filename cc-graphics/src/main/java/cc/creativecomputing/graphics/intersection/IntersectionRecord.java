...
public class IntersectionRecord {
	...
	private static final class Intersection implements Comparable<Intersection> {
		...
		private final CCVector3 _point;
		private final CCVector3 _normal;
		private final PrimitiveKey _primitiveKey;

		private Intersection(final double distance, final CCVector3 point, final CCVector3 normal, final PrimitiveKey primitiveKey) {
			_distance = distance;
			_point = point;
			_normal = normal;
	 ...
	 */
	public IntersectionRecord(final double[] distances, final CCVector3[] points) {
	 ...
	 * @throws Ardor3dException
	 *             if distances.length != points.length or points.length !=
	 *             primitives.size() (if primitives is not null)
	 */
	public IntersectionRecord(final double[] distances, final CCVector3[] points, final List<PrimitiveKey> primitives) {
	 ...
	 *             primitives.size() (if primitives is not null)
	 */
	public IntersectionRecord(final double[] distances, final CCVector3[] points, final CCVector3[] normals,
		...
		if (distances.length != points.length || (primitives != null && points.length != primitives.size())
			...
			throw new RuntimeException("All arguments must have an equal number of elements.");
		}
	 ...
	 */
	public CCVector3 getIntersectionPoint(final int index) {
	 ...
	 */
	public CCVector3 getIntersectionNormal(final int index) {
	...
	/**
	 * @return the smallest distance in the distance array or -1 if there are no
	 *         distances in this.
	 */
	public double getClosestDistance() {
		final int i = getClosestIntersection();
		return i != -1 ? _intersections[i]._distance : -1.0f;
	}

	/**
	 * @return the largest distance in the distance array or -1 if there are no
	 *         distances in this.
	 */
	...
	/**
	 * @return the index in this record with the smallest relative distance or
	 *         -1 if there are no distances in this record.
	 */
	...
	/**
	 * @return the index in this record with the largest relative distance or -1
	 *         if there are no distances in this record.
	 */
