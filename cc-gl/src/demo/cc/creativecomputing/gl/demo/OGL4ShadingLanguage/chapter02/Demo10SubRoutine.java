...
import cc.creativecomputing.gl4.GLShaderProgram.GLSubroutineUniformInfo;
...
public class Demo10SubRoutine extends CCGL4Adapter{
	...
	public void init(GLGraphics g) {
		...
		_myMesh =  new GLMesh(GLDrawMode.TRIANGLES, new GLCombinedBuffer(new CCTorus(10, 10, 0.3f,0.7f).data()));

	    _myShader = new GLShaderProgram(
	    	new GLShaderObject(GLShaderType.VERTEX, CCNIOUtil.classPath(this, "shader/subroutine_vert.glsl")),
	    	new GLShaderObject(GLShaderType.FRAGMENT, CCNIOUtil.classPath(this, "shader/subroutine_frag.glsl"))
		);
	    _myShader.use();
	    
	    for(GLSubroutineUniformInfo mySubroutineUniform:_myShader.subroutineUniforms(GLShaderType.VERTEX)){
	    	CCLog.info(mySubroutineUniform);
	    }
	...
	public void display(GLGraphics g) {
		...
		_myModelMatrix.applyTranslationPost(-0.4f,0.0f,0.0f);
		_myModelMatrix.applyRotationY(_myAngle);
		setMatrices();
		_myShader.subroutine(GLShaderType.VERTEX, "shadeModel", "phongModel");
	    _myMesh.draw();
	    
	    _myModelMatrix.set(CCMatrix4x4.IDENTITY);
		_myModelMatrix.applyTranslationPost( 0.4f,0,0);
		_myModelMatrix.applyRotationY(_myAngle);
		setMatrices();
		_myShader.subroutine(GLShaderType.VERTEX, "shadeModel", "diffuseOnly");
	    _myMesh.draw();
	}
	
	public static void main(String[] args) {
		CCGL4Application myAppManager = new CCGL4Application(new Demo10SubRoutine());
