...
public class Demo14PerFragment extends CCGL4Adapter{
...
//	    _myPlane = new CCMesh(new CCGridXZ(50.0f, 50.0f, true).generateMesh(2, 2));
//	    _myTeapot = new CCMesh(new CCTeapot().generateMesh(14));
	...
	public void init(GLGraphics theG) {
		...
		_myTorus = new GLMesh(GLDrawMode.TRIANGLES, new GLCombinedBuffer(new CCTorus(100, 40, 0.2f, 0.6f).data()));

		_myShader = new GLShaderProgram(
			new GLShaderObject(GLShaderType.VERTEX, CCNIOUtil.classPath(this, "shader/perfrag_vert.glsl")),
			new GLShaderObject(GLShaderType.FRAGMENT, CCNIOUtil.classPath(this, "shader/perfrag_frag.glsl"))
		);
		_myShader.use();

		_myShader.uniform3f("LightIntensity", 0.9f, 0.9f, 0.9f);
	}
	...
	public void display(GLGraphics g) {
		...
		_myShader.uniform4f("LightPosition", _myViewMatrix.applyPost(new CCVector4(10.0f * CCMath.cos(_myAngle), 3.0f, 10.0f * CCMath.sin(_myAngle), 1.0f)));
		...
		_myModelMatrix.applyTranslationPost(0.0f, 0.0f, 0.0f);
...
//		_myPlane.draw();
	...
	public static void main(String[] args) {
		CCGL4Application myAppManager = new CCGL4Application(new Demo14PerFragment());
