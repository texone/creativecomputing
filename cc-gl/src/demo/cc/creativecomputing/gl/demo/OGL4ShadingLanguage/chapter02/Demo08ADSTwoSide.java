...
public class Demo08ADSTwoSide extends CCGL4Adapter{
	...
	public void init(GLGraphics g) {
		...
		_myViewMatrix = CCMatrix4x4.createLookAt(new CCVector3(2.0f,4.0f,2.0f), new CCVector3(0.0f,0.0f,0.0f), new CCVector3(0.0f,1.0f,0.0f));
		_myProjectionMatrix = new CCMatrix4x4();
		  
		CCMatrix4x4 transform = new CCMatrix4x4();
		transform.applyTranslationPost(0.0f,1.5f,0.25f);
	    _myTorus =  new GLMesh(GLDrawMode.TRIANGLES, new GLCombinedBuffer(new CCTorus(10, 10, 0.3f,0.7f).data()));
//		_myTeapot = new CCMesh(new CCTorus(0.3f, 0.7f).generateMesh(30, 30));

	    _myShader = new GLShaderProgram(
	    	new GLShaderObject(GLShaderType.VERTEX, CCNIOUtil.classPath(this, "shader/twoside_vert.glsl")),
	    	new GLShaderObject(GLShaderType.FRAGMENT, CCNIOUtil.classPath(this, "shader/twoside_frag.glsl"))
		);
	    _myShader.use();
...
//		_myModelMatrix.applyTranslationPost(0,-1,0);
		_myModelMatrix.applyRotationX(CCMath.radians( 90f));
		_myModelMatrix.applyRotationY(CCMath.radians( 140));    
	}
	...
	public static void main(String[] args) {
		CCGL4Application myAppManager = new CCGL4Application(new Demo08ADSTwoSide());
