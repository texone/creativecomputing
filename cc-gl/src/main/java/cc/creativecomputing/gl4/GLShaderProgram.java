...
public class GLShaderProgram{
	...
	public static class CCGLShaderProgramPipeline{
		...
		public CCGLShaderProgramPipeline(){
			...
			IntBuffer myPipelineBuffer = IntBuffer.allocate(1);
			gl.glGenProgramPipelines(1, myPipelineBuffer);
			_myID = myPipelineBuffer.get(0);
		}
		
		public void bind(){
			GL4 gl = GLGraphics.currentGL();
			gl.glBindProgramPipeline(_myID);
		}
	...
	public void attach(GLShaderObject theShaderObject){
		GL4 gl = GLGraphics.currentGL();
		gl.glAttachShader(_myID, theShaderObject.id());
	}
	...
	public void detach(GLShaderObject theShaderObject){
		GL4 gl = GLGraphics.currentGL();
		gl.glDetachShader(_myID, theShaderObject.id());
	}
	...
	public String infoLog(){
		...
		gl.glGetProgramInfoLog(_myID, length, null, infoLog);
	...
	public static enum GLProgramParameter {
		 ...
		 */
		ACTIVE_ATTRIBUTE_MAX_LENGTH(GL4.GL_ACTIVE_ATTRIBUTE_MAX_LENGTH),
		/**
		 * returns the number of active uniform variables for program.
		 */
		ACTIVE_UNIFORMS(GL4.GL_ACTIVE_UNIFORMS),
		/**
		 ...
		 */
		ACTIVE_UNIFORM_MAX_LENGTH(GL4.GL_ACTIVE_UNIFORM_MAX_LENGTH),
		/**
		 ...
		 */
		PROGRAM_BINARY_LENGTH(GL4.GL_PROGRAM_BINARY_LENGTH),
		/*
...
//		COMPUTE_WORK_GROUP_SIZE(GL4.GL_COMPUTE_WORK_GROUP_SIZE),
		 ...
		 */
		GEOMETRY_OUTPUT_TYPE(GL4.GL_GEOMETRY_OUTPUT_TYPE);
		...
		public static GLProgramParameter fromGLID(int theGLID){
			switch(theGLID){
			case GL4.GL_DELETE_STATUS:return DELETE_STATUS;
			case GL4.GL_LINK_STATUS:return LINK_STATUS;
			case GL4.GL_VALIDATE_STATUS:return VALIDATE_STATUS;
			case GL4.GL_INFO_LOG_LENGTH:return INFO_LOG_LENGTH;
			case GL4.GL_ATTACHED_SHADERS:return ATTACHED_SHADERS;
			case GL4.GL_ACTIVE_ATOMIC_COUNTER_BUFFERS:return ACTIVE_ATOMIC_COUNTER_BUFFERS;
			case GL4.GL_ACTIVE_ATTRIBUTES:return ACTIVE_ATTRIBUTES;
			case GL4.GL_ACTIVE_ATTRIBUTE_MAX_LENGTH:return ACTIVE_ATTRIBUTE_MAX_LENGTH;
			case GL4.GL_ACTIVE_UNIFORMS:return ACTIVE_UNIFORMS;
			case GL4.GL_ACTIVE_UNIFORM_MAX_LENGTH:return ACTIVE_UNIFORM_MAX_LENGTH;
			case GL4.GL_PROGRAM_BINARY_LENGTH:return PROGRAM_BINARY_LENGTH;
//			case GL4.GL_COMPUTE_WORK_GROUP_SIZE:return COMPUTE_WORK_GROUP_SIZE;
			case GL4.GL_TRANSFORM_FEEDBACK_BUFFER_MODE:return TRANSFORM_FEEDBACK_BUFFER_MODE;
			case GL4.GL_TRANSFORM_FEEDBACK_VARYINGS:return TRANSFORM_FEEDBACK_VARYINGS;
			case GL4.GL_TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH:return TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH;
			case GL4.GL_GEOMETRY_VERTICES_OUT:return GEOMETRY_VERTICES_OUT;
			case GL4.GL_GEOMETRY_INPUT_TYPE:return GEOMETRY_INPUT_TYPE;
			case GL4.GL_GEOMETRY_OUTPUT_TYPE:return GEOMETRY_OUTPUT_TYPE;
			}
			return null;
		}
	}
	...
	private int glGetProgram(GLProgramParameter theParameter){
		GL4 gl = GLGraphics.currentGL();
		gl.glGetProgramiv(_myID, theParameter.glID(), GLBufferUtil.intBuffer());
	...
	public boolean deleteStatus(){
		return glGetProgram(GLProgramParameter.DELETE_STATUS) == GL4.GL_TRUE;
	}
	...
	public boolean linkStatus(){
		return glGetProgram(GLProgramParameter.LINK_STATUS) == GL4.GL_TRUE;
	}
	...
	public boolean validateStatus(){
		return glGetProgram(GLProgramParameter.VALIDATE_STATUS) == GL4.GL_TRUE;
	}
...
//	COMPUTE_WORK_GROUP_SIZE(GL4.GL_COMPUTE_WORK_GROUP_SIZE),
	...
	public static enum GLTransformFeedbackMode {
		...
		public static GLTransformFeedbackMode fromGLID(int theGLID){
			switch(theGLID){
			case GL4.GL_INTERLEAVED_ATTRIBS:return INTERLEAVED_ATTRIBS;
			case GL4.GL_SEPARATE_ATTRIBS:return SEPARATE_ATTRIBS;
			}
			return null;
		}
	}
	...
	public int attribLocation(String theName){
		GL4 gl = GLGraphics.currentGL();
		return gl.glGetAttribLocation(_myID, theName);
	}
	...
	public GLAttributeInfo activeAttribute(int theIndex){
		...
		ByteBuffer nameBuffer = CCBufferUtil.newByteBuffer(maxSize);
		IntBuffer lengthBuffer = CCBufferUtil.newIntBuffer(1);
		IntBuffer sizeBuffer = CCBufferUtil.newIntBuffer(1);
		IntBuffer typeBuffer = CCBufferUtil.newIntBuffer(1);
		gl.glGetActiveAttrib(_myID, theIndex, maxSize, lengthBuffer, sizeBuffer, typeBuffer, nameBuffer);
		
		int myStringLength = lengthBuffer.get();
		int myType = typeBuffer.get(0);

		byte[] infoBytes = new byte[myStringLength];
		nameBuffer.get(infoBytes);
		String myAttributeName = new String(infoBytes);
		int myLocation = gl.glGetAttribLocation(_myID, myAttributeName);
	...
	public GLUniformInfo activeUniform(int theIndex){
		...
		IntBuffer sizeBuffer = CCBufferUtil.newIntBuffer(1);
		IntBuffer typeBuffer = CCBufferUtil.newIntBuffer(1);
		gl.glGetActiveUniform(_myID, theIndex, maxSize, lengthBuffer, sizeBuffer, typeBuffer, nameBuffer);
		...
		byte[] infoBytes = new byte[myStringLength];
		nameBuffer.get(infoBytes);
		String myUniformName = new String(infoBytes);
		int myLocation = gl.glGetUniformLocation(_myID, myUniformName);
	...
	public static enum GLProgramStageQuery{
		 ...
		 */
		ACTIVE_SUBROUTINE_UNIFORMS(GL4.GL_ACTIVE_SUBROUTINE_UNIFORMS),
		/**
		 * number of active subroutine variable locations in the stage
		 */
		ACTIVE_SUBROUTINE_UNIFORM_LOCATIONS(GL4.GL_ACTIVE_SUBROUTINE_UNIFORM_LOCATIONS),
		/**
		 * number of active subroutines in the stage
		 */
		ACTIVE_SUBROUTINES(GL4.GL_ACTIVE_SUBROUTINES),
		/**
		 * length of the longest subroutine uniform for the stage
		 */
		ACTIVE_SUBROUTINE_UNIFORM_MAX_LENGTH(GL4.GL_ACTIVE_SUBROUTINE_UNIFORM_MAX_LENGTH),
		/**
		 *  length of the longest subroutine name for the stage
		 */
		ACTIVE_SUBROUTINE_MAX_LENGTH(GL4.GL_ACTIVE_SUBROUTINE_MAX_LENGTH);
		...
		public static GLProgramStageQuery fromGLID(int theGLID){
			switch(theGLID){
			case GL4.GL_ACTIVE_SUBROUTINE_UNIFORMS:return ACTIVE_SUBROUTINE_UNIFORMS;
			case GL4.GL_ACTIVE_SUBROUTINE_UNIFORM_LOCATIONS:return ACTIVE_SUBROUTINE_UNIFORM_LOCATIONS;
			case GL4.GL_ACTIVE_SUBROUTINES:return ACTIVE_SUBROUTINES;
			case GL4.GL_ACTIVE_SUBROUTINE_UNIFORM_MAX_LENGTH:return ACTIVE_SUBROUTINE_UNIFORM_MAX_LENGTH;
			case GL4.GL_ACTIVE_SUBROUTINE_MAX_LENGTH:return ACTIVE_SUBROUTINE_MAX_LENGTH;
			}
			return null;
		}
	} 
	...
	public int programStage(GLShaderType theShaderType, GLProgramStageQuery theQuery){
		GL4 gl = GLGraphics.currentGL();
		gl.glGetProgramStageiv(_myID, theShaderType.glID, theQuery.glID(), GLBufferUtil.intBuffer());
		return GLBufferUtil.intBuffer().get();
	}
	...
	private class GLSubroutineManager{
		...
		private void apply(){
			...
			CCLog.info(_myActiveSubrountineUniforms);
			uniformSubroutines(_myShaderType, _myActiveSubrountineUniforms, _mySubroutineIndices);
		}
	}
	...
	public void uniformSubroutines(GLShaderType theShaderType, int theCount, IntBuffer theIndices){
		GL4 gl = GLGraphics.currentGL();
		gl.glUniformSubroutinesuiv(theShaderType.glID, theCount, theIndices);
	}
	...
	public GLSubroutineUniformInfo activeSubroutine(int theID, GLShaderType theShaderType){
		...
		GL4 gl = GLGraphics.currentGL();
	    gl.glGetActiveSubroutineUniformName(_myID, theShaderType.glID, theID, maxSize, myLengthBuffer, myNameBuffer);
	    
		int myStringLength = myLengthBuffer.get();
		byte[] infoBytes = new byte[myStringLength];
		myNameBuffer.get(infoBytes);
		String mySubroutineUniformName = new String(infoBytes);
		int mySubroutineLocation = gl.glGetSubroutineUniformLocation(_myID, theShaderType.glID, mySubroutineUniformName);
		
		GLSubroutineUniformInfo mySubroutineUniformInfo = new GLSubroutineUniformInfo(mySubroutineLocation, mySubroutineUniformName);
		
		gl.glGetActiveSubroutineUniformiv(_myID, theShaderType.glID, theID, GL4.GL_NUM_COMPATIBLE_SUBROUTINES, GLBufferUtil.intBuffer());
		int myNumberOfCompatibleSubroutines = GLBufferUtil.intBuffer().get();
		
		IntBuffer mySubroutines = IntBuffer.allocate(myNumberOfCompatibleSubroutines);		   
	    gl.glGetActiveSubroutineUniformiv(_myID, theShaderType.glID, theID, GL4.GL_COMPATIBLE_SUBROUTINES, mySubroutines);
	    ...
	    for (int i=0; i < myNumberOfCompatibleSubroutines; i++) {
	        ...
	        gl.glGetActiveSubroutineName(_myID, theShaderType.glID, mySubroutines.get(i), myMaxSubroutineNameLength, mySubroutineNameLengthBuffer, mySubroutineNameBuffer);
			...
			int mySubroutineIndex = gl.glGetSubroutineIndex(_myID, theShaderType.glID, mySubroutineName);
	...
	public int uniformLocation(String theUniform){
		GL4 gl = GLGraphics.currentGL();
		return gl.glGetUniformLocation(_myID, theUniform);
	}
	...
	public GLUniformBlock uniformBlock(String theBlockName, String...theUniformNamess){
		if(!_myUniformBlockMap.containsKey(theBlockName)){
			GL4 gl = GLGraphics.currentGL();
			int myID = gl.glGetUniformBlockIndex(_myID, theBlockName);
			if(myID == GL4.GL_INVALID_INDEX){
	...
	public void uniform1i(final int theLocation, final int theValue){
		GL4 gl = GLGraphics.currentGL();
		gl.glProgramUniform1i(_myID, theLocation, theValue);
	}
	...
	public void uniform(final int theLocation, final boolean theValue){
		GL4 gl = GLGraphics.currentGL();
		if(theValue)gl.glProgramUniform1i(_myID, theLocation, 1);
		else gl.glProgramUniform1i(_myID, theLocation, 0);
	}
	...
	public void uniform1f(final int theLocation, final float theValue){
		GLGraphics.currentGL().glProgramUniform1f(_myID, theLocation, theValue);
	}
	...
	public void uniform1fv(final int theLocation, final List<?> theVectors){
		...
		myData.rewind();
		GLGraphics.currentGL().glProgramUniform1fv(_myID, theLocation, theVectors.size(), myData);
	}
	...
	public void uniform1fv(final int theLocation, float...theValues){
		...
		myData.rewind();
		GLGraphics.currentGL().glProgramUniform1fv(_myID, theLocation, theValues.length, myData);
	}
	...
	public void uniform2f(final int theLocation, final float theX, final float theY){
		GLGraphics.currentGL().glProgramUniform2f(_myID, theLocation, theX, theY);
	}
	...
	public void uniform3f(final int theLocation, final float theX, final float theY, final float theZ){
		GLGraphics.currentGL().glProgramUniform3f(_myID, theLocation, theX, theY, theZ);
	}
	...
	public void uniform4f(int theLocation, final float theX, final float theY, final float theZ, float theW){
		GLGraphics.currentGL().glProgramUniform4f(_myID, theLocation, theX, theY, theZ, theW);
	}
	...
	public void uniform4fv(final int theLocation, final List<?> theVectors){
		...
		myData.rewind();
		GLGraphics.currentGL().glProgramUniform4fv(_myID, theLocation, theVectors.size(), myData);
	}
	...
	public void uniform(final int theLocation, final CCColor theColor){
		GL4 gl = GLGraphics.currentGL();
		gl.glProgramUniform4f(_myID, theLocation, (float)theColor.red(), (float)theColor.green(), (float)theColor.blue(), (float)theColor.alpha());
	}
	...
	public void uniformMatrix3f(final int theLocation, FloatBuffer theMatrix) { 
		theMatrix.rewind();
		GL4 gl = GLGraphics.currentGL();
		gl.glProgramUniformMatrix3fv(_myID, theLocation, 1, false, theMatrix);
	}
	
	public void uniformMatrix3f(final int theLocation, CCMatrix3x3 theMatrix) { 
		uniformMatrix3f(theLocation, theMatrix.toBuffer(GLBufferUtil.wrapBuffer(9)));
	}
	...
	public void uniformMatrix4f(final int theLocation, FloatBuffer theMatrix) { 
		theMatrix.rewind();
		GLGraphics.currentGL().glProgramUniformMatrix4fv(_myID, theLocation, 1, false, theMatrix);
	}
	
	public void uniformMatrix4f(final String theName, FloatBuffer theMatrix) { 
		theMatrix.rewind();
		GLGraphics.currentGL().glProgramUniformMatrix4fv(_myID, uniformLocation(theName), 1, false, theMatrix);
	}
	
	public void uniformMatrix4f(final int theLocation, float[] theMatrix) { 
		GLGraphics.currentGL().glProgramUniformMatrix4fv(_myID, theLocation, 1, false, theMatrix, 0);
	}
	
	public void uniformMatrix4f(final String theName, float[] theMatrix) { 
		GLGraphics.currentGL().glProgramUniformMatrix4fv(_myID, uniformLocation(theName), 1, false, theMatrix, 0);
	}
	
	public void uniformMatrix4f(final int theLocation, CCMatrix4x4 theMatrix) { 
		uniformMatrix4f(theLocation, theMatrix.toFloatBuffer(GLBufferUtil.wrapBuffer(16)));
	}
	...
	public void uniformMatrix4fv(final int theLocation, final List<CCMatrix4x4> theMatrices){
		...
		for(CCMatrix4x4 myMatrix:theMatrices){
			...
			myData.put(myMatrix.toFloatBuffer(GLBufferUtil.wrapBuffer(16)));
		}
		myData.rewind();
		GLGraphics.currentGL().glProgramUniformMatrix4fv(_myID, theLocation, theMatrices.size(), false, myData);
	}
