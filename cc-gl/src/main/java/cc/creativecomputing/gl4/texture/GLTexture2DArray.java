...
public class GLTexture2DArray extends GLTexture{
	...
	public GLTexture2DArray(int theMipMapLevel, GLPixelDataInternalFormat theFormat, int theWidth, int theHeight, int theLayers){
		super(GLTextureTarget.TEXTURE_2D_ARRAY);
		bind();
		texStorage( theMipMapLevel, theFormat, theWidth, theHeight, theLayers);
	}
	 ...
	 */
	public void texStorage(int theLevels, GLPixelDataInternalFormat theFormat, int theWidth, int theHeight, int theLayer){
		GL4 gl = GLGraphics.currentGL();
		gl.glTexStorage3D(_myTarget.glID(), theLevels, theFormat.glID(), theWidth, theHeight, theLayer);
	}
	...
	public void texSubImage(
		...
		int theWidh, int theHeight, int theLayer,
		GLPixelDataFormat theFormat, GLPixelDataType theType, 
		Buffer thePixels
	) {
		GL4 gl = GLGraphics.currentGL();
		gl.glTexSubImage3D(_myTarget.glID(), theLevel, theXOffset, theYOffset, theZOffset, theWidh, theHeight, theLayer, theFormat.glID(), theType.glID(), thePixels);
	}
	...
	public void texSubImage(
		...
		GL4 gl = GLGraphics.currentGL();
		gl.glTexSubImage3D(_myTarget.glID(), theLevel, theXOffset, theYOffset, theZOffset, theWidh, theHeight, theLayer, theFormat.glID(), theType.glID(), theOffset);
	}
}
