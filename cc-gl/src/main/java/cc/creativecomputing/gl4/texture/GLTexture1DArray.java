...
public class GLTexture1DArray extends GLTexture{
	
	public GLTexture1DArray(int theMipMapLevel, GLPixelDataInternalFormat theFormat, int theWidth, int theLayers){
		super(GLTextureTarget.TEXTURE_1D_ARRAY);
		bind();
		texStorage( theMipMapLevel, theFormat, theWidth, theLayers);
	}
	 ...
	 * @param theLayer Specifies the layer of the texture, in texels.
	 */
	public void texStorage(int theLevels, GLPixelDataInternalFormat theFormat, int theWidth, int theLayer){
		GL4 gl = GLGraphics.currentGL();
		gl.glTexStorage2D(_myTarget.glID(), theLevels, theFormat.glID(), theWidth, theLayer);
	}
	...
	public void texSubImage(
		int theLevel, 
		int theXOffset, int theLayerOffset, 
		int theWidh, int theLayers,
		GLPixelDataFormat theFormat, GLPixelDataType theType, 
		Buffer thePixels
	) {
		GL4 gl = GLGraphics.currentGL();
		gl.glTexSubImage2D(_myTarget.glID(), theLevel, theXOffset, theLayerOffset, theWidh, theLayers, theFormat.glID(), theType.glID(), thePixels);
	}
	...
	public void texSubImage(
		...
		GL4 gl = GLGraphics.currentGL();
		gl.glTexSubImage2D(_myTarget.glID(), theLevel, theXOffset, theLayerOffset, theWidh, theLayers, theFormat.glID(), theType.glID(), theOffset);
	}
}
