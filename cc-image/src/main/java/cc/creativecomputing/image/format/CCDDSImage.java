...
public class CCDDSImage {
	 ...
	 */
	public static CCDDSImage read(Path thePath) {
		try {
			CCDDSImage image = new CCDDSImage();
			image.readFromFile(thePath);
			return image;
		} catch (IOException e) {
	...
	public static boolean isDDSImage(InputStream in){
		...
		if (!in.markSupported()) {
			throw new CCImageException("Can not test non-destructively whether given InputStream is a DDS image");
		}
		in.mark(4);
		int magic = 0;
		...
		try {
			...
			throw new CCImageException("Can not test non-destructively whether given InputStream is a DDS image", e);
		}
		return (magic == MAGIC);
	}
	...
	public void write(Path thePath){
		try {
			FileChannel chan = FileChannel.open(thePath, StandardOpenOption.WRITE, StandardOpenOption.CREATE_NEW);
	...
	private void readFromFile(Path thePath) throws IOException {
		chan = FileChannel.open(thePath, StandardOpenOption.READ);
