...
public class CCKinectDepthDemo extends CCKinectGLAdapter {
	
		private CCKinectImage _myDepthImage;
		
		@Override
		public void start(CCKinectModule theKinectModule) {
			_myDepthImage = theKinectModule.colorPositionImage();
		}
		...
		public void init(GLGraphics g) {
			...
			_myPrimitive = new CCQuad(_myDepthImage.width(), _myDepthImage.height());
			_myPrimitive.effect(new CCSingleTextureEffect(_myTexture));
//			_myPrimitive.effect(new CCSolidColorEffect(new CCColor(255)));
...
//			myBuffer.rewind();
		...
		public static void main(String[] args) {
			CCKinectGraphicsApplication myAppManager = new CCKinectGraphicsApplication(new CCKinectDepthDemo());
