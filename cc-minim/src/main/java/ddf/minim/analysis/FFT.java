...
public class FFT extends FourierTransform {
	...
	public void scaleBand(int i, float s) {
		if (s < 0) {
			throw new CCSoundException("Can't scale a frequency band by a negative value.");
		}
	...
	public void setBand(int i, float a) {
		if (a < 0) {
			throw new CCSoundException("Can't set a frequency band to a negative value.");
		}
	...
	public void forward(float[] buffer) {
		if (buffer.length != timeSize) {
			throw new CCSoundException("FFT.forward: The length of the passed sample buffer must be equal to timeSize().");
		}
		doWindow(buffer);
	...
	public void forward(float[] buffer, int startAt) {
		if (buffer.length - startAt < timeSize) {
			throw new CCSoundException("FourierTransform.forward: not enough samples in the buffer between "
							+ startAt
							+ " and "
							+ buffer.length
	...
	public void forward(float[] buffReal, float[] buffImag) {
		if (buffReal.length != timeSize || buffImag.length != timeSize) {
			throw new CCSoundException("FFT.forward: The length of the passed buffers must be equal to timeSize().");
		}
	...
	public void inverse(float[] buffer) {
		if (buffer.length > real.length) {
			throw new CCSoundException("FFT.inverse: the passed array's length must equal FFT.timeSize().");
		}
		// conjugate
