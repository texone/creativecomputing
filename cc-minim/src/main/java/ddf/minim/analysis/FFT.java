...
public class FFT extends FourierTransform
{
  ...
  public void scaleBand(int i, float s)
  {
    if (s < 0)
    {
      CCSoundIO.error("Can't scale a frequency band by a negative value.");
      return;
    }
  ...
  public void setBand(int i, float a)
  {
    if (a < 0)
    {
      CCSoundIO.error("Can't set a frequency band to a negative value.");
      return;
    }
  ...
  public void forward(float[] buffer, int startAt)
  {
	  ...
	  {
		  CCSoundIO.error( "FourierTransform.forward: not enough samples in the buffer between " + 
