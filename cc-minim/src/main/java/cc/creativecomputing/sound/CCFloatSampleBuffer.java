...
public class CCFloatSampleBuffer {
	...
	private float ditherBits = CCFloatSampleTools.DEFAULT_DITHER_BITS;
	 ...
	 */
	public CCFloatSampleBuffer(int channelCount, int sampleCount, float sampleRate) {
	 ...
	 */
	public CCFloatSampleBuffer(byte[] buffer, int offset, int byteCount,
			AudioFormat format) {
	...
	public static void checkFormatSupported(AudioFormat format) {
		CCFloatSampleTools.getFormatType(format);
	}
	...
	public void initFromByteArray(byte[] buffer, int offset, int byteCount,
		...
		originalFormatType = CCFloatSampleTools.getFormatType(format);

		CCFloatSampleTools.byte2float(buffer, offset, channels, 0, sampleCount,
				format);
	}
	 ...
	 */
	public void initFromFloatSampleBuffer(CCFloatSampleBuffer source) {
	...
	public int writeByteBuffer(byte[] buffer, int srcByteOffset,
		...
		for (int channel = 0; channel < lChannels; channel++) {
			...
			CCFloatSampleTools.byte2floatGeneric(buffer, srcByteOffset,
	...
	public int convertToByteArray(int readOffset, int lenInSamples,
		...
		if (format != lastConvertToByteArrayFormat) {
			...
			lastConvertToByteArrayFormatCode = CCFloatSampleTools.getFormatType(format);
		}
		CCFloatSampleTools.float2byte(channels, readOffset, buffer, writeOffset,
	 ...
	 */
	public void mix(CCFloatSampleBuffer source) {
	 ...
	 */
	public void mix(CCFloatSampleBuffer source, int sourceOffset, int thisOffset, int count) {
		int localChannelCount = getChannelCount();
		for (int ch = 0; ch < localChannelCount; ch++) {
			float[] thisChannel = getChannel(ch);
			float[] otherChannel = source.getChannel(ch);
	 ...
	 */
	public int copyTo(CCFloatSampleBuffer dest, int destOffset, int count) {
	 ...
	 * @return the number of samples copied
	 */
	public int copyTo(int srcOffset, CCFloatSampleBuffer dest, int destOffset, int count) {
	...
	public void setSamplesFromBytes(byte[] input, int inByteOffset,
		...
		CCFloatSampleTools.byte2float(input, inByteOffset, channels, floatOffset,
	...
	protected float getConvertDitherBits(int newFormatType) {
		...
		case DITHER_MODE_AUTOMATIC:
			doDither = (originalFormatType & CCFloatSampleTools.F_SAMPLE_WIDTH_MASK) > (newFormatType & CCFloatSampleTools.F_SAMPLE_WIDTH_MASK);
			break;
		case DITHER_MODE_ON:
			doDither = true;
			break;
