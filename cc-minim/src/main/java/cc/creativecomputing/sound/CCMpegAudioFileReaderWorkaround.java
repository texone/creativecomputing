 ...
 * want to lug along jlgui just to be able to play mp3 files in the web browser.
 * So I include it here, with slight modifications for error/debug reporting.
 * 
 */
final class CCMpegAudioFileReaderWorkaround extends CCMpegAudioFileReader {
	CCMpegAudioFileReaderWorkaround() {
		super();
	}
	...
	public AudioInputStream getAudioInputStream(URL url, String userAgent) throws UnsupportedAudioFileException,
		...
		CCSoundIO.debug("Base input stream is: " + conn.getInputStream().toString());
		...
		if (isShout == true) {
			// Yes
			CCSoundIO.debug("URL is a shoutcast server.");
			...
			if (metaint != null) {
				...
				CCSoundIO.debug("URL is probably an icecast 2 mp3 stream");
				IcyInputStream icyStream = new IcyInputStream(bInputStream, metaint);
				icyStream.addTagParseListener(IcyListener.getInstance());
				inputStream = icyStream;
			} else {
				CCSoundIO.debug("URL is not shoutcast or icecast 2.");
				inputStream = bInputStream;
			}
		}
		AudioInputStream audioInputStream = null;
		try {
			CCSoundIO.debug("Attempting to get audioInputStream.");
		...
		CCSoundIO.debug("MpegAudioFileReaderWorkaround.getAudioInputStream(URL,String): end");
