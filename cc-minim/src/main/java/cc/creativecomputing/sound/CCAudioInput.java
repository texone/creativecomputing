...
final class CCAudioInput extends Thread implements CCAudioStream {
	...
	CCAudioInput(TargetDataLine tdl, int bufferSize) {
		line = tdl;
		this.bufferSize = bufferSize;
		buffer = new CCFloatSampleBuffer(tdl.getFormat().getChannels(), bufferSize, tdl.getFormat().getSampleRate());
		finished = false;
		mono = (buffer.getChannelCount() == 1);
		int byteBufferSize = buffer.getByteArrayBufferSize(line.getFormat());
		CCSoundIO.debug("byteBufferSize is " + byteBufferSize);
	...
	public void run() {
		...
		while (!finished) {
			...
			buffer.setSamplesFromBytes(rawBytes, 0, line.getFormat(), 0, buffer.getSampleCount());
	...
	public float[] read() {
		// TODO: this is sort of terrible, but will do for now. would be much
		// better
		// to dig the conversion stuff out of FloatSampleBuffer and do this more
		// directly
		int numSamples = 1;
	...
	public int read(CCMultiChannelBuffer buffer) {
		...
		CCFloatSampleBuffer convert = new CCFloatSampleBuffer(numChannels, numSamples, sampleRate);
