 ...
 */
class CCMpegAudioFileReader extends TAudioFileReader {
	...
	private static final String[] id3v1genres = { "Blues", "Classic Rock", "Country", "Dance", "Disco", "Funk",
			...
			"Freestyle", "Duet", "Punk Rock", "Drum Solo", "A Capela", "Euro-House", "Dance Hall", "Goa",
	...
	public AudioFileFormat getAudioFileFormat(InputStream inputStream, long mediaLength)
		...
		CCSoundIO.debug("MpegAudioFileReader.getAudioFileFormat(InputStream inputStream, long mediaLength): begin");
		...
		pis.read(head);
		CCSoundIO.debug("InputStream : " + inputStream + " =>" + new String(head));
		...
		if ((head[0] == 'R') && (head[1] == 'I') && (head[2] == 'F') && (head[3] == 'F') && (head[8] == 'W')
			...
			CCSoundIO.debug("RIFF/WAV stream found");
			...
			CCSoundIO.debug("AU stream found");
			if (weak == null)
			...
			CCSoundIO.debug("FLAC stream found");
			if (weak == null)
		...
		// Ogg stream ?
				...
				&& ((head[2] == 'G') | (head[2] == 'g'))) {
			CCSoundIO.debug("Ogg stream found");
			if (weak == null)
		...
		AudioFormat format = new MpegAudioFormat(encoding, (float) nFrequency, AudioSystem.NOT_SPECIFIED // SampleSizeInBits
																											// -
																											// The
																											// size
																											// of
	...
	public AudioInputStream getAudioInputStream(URL url) throws UnsupportedAudioFileException, IOException {
		...
		if ((read > 2)
				&& (((head[0] == 'I') | (head[0] == 'i')) && ((head[1] == 'C') | (head[1] == 'c')) && ((head[2] == 'Y') | (head[2] == 'y'))))
			isShout = true;
		bInputStream.reset();
		...
		CCSoundIO.debug("MpegAudioFileReader.getAudioInputStream(URL): end");
	 ...
	 */
	public AudioInputStream getAudioInputStream(InputStream inputStream) throws UnsupportedAudioFileException,
			IOException {
		CCSoundIO.debug("MpegAudioFileReader.getAudioInputStream(InputStream inputStream)");
	...
	protected String parseComment(byte[] bframes, int offset, int size) {
		...
		try {
			...
			// there should only be one, but the mp3 tagger Mp3Tag puts in more
			// than one
			while (bframes[offset + skip] == 0 && skip < size) {
				skip += 1;
			}
