 ...
 */
public class TriggerASample extends CCApp {
	...
	public void setup() {
		...
		kick = minim.loadSample(
			CCIOUtil.dataPath("BD.mp3"), // filename
			512 // buffer size
		);
		...
		snare = minim.loadSample(CCIOUtil.dataPath("SD.wav"), 512);
		if (snare == null)
	...
	public void draw() {
		...
		for (int i = 0; i < kick.bufferSize() - 1; i++) {
			float x1 = CCMath.map(i, 0, kick.bufferSize(), 0, width);
			float x2 = CCMath.map(i + 1, 0, kick.bufferSize(), 0, width);
	...
	public static void main(String[] args) {
		CCApplicationManager myManager = new CCApplicationManager(TriggerASample.class);
