...
public class CCVBOMesh extends CCMesh {
	...
	public CCVBOMesh(final List<CCVector3> theVertices, final List<CCVector2> theTextureCoords, final List<CCColor> theColors) {
		super(theVertices, theTextureCoords, theColors);
	}

	public CCVBOMesh(final CCDrawMode theDrawMode, final List<CCVector3> theVertices,
			final List<CCVector2> theTextureCoords, final List<CCColor> theColors) {
	...
	public void prepareVertexData(int theNumberOfVertices, int theVertexSize) {
		...
		if (_myVertices == null || _myVertices.limit() / _myVertexSize != _myNumberOfVertices) {
			...
			if (_myVertexBuffer == null) {
				...
				_myVertexBuffer.bufferData(_myNumberOfVertices * _myVertexSize * CCBufferUtil.SIZE_OF_FLOAT, null,
						CCUsageFrequency.DYNAMIC, CCUsageTYPE.DRAW);
			}
	...
	public void vertices(CCShaderBuffer theShaderTexture, final int theID, final int theX, final int theY, final int theWidth, final int theHeight) {
		if (_myVertexBuffer == null)
			_myVertexBuffer = new CCBufferObject();
	...
	public void normals(CCShaderBuffer theShaderTexture, final int theID, final int theX, final int theY, final int theWidth, final int theHeight) {
		if (_myNormalBuffer == null)
			_myNormalBuffer = new CCBufferObject();
	...
	public void prepareTextureCoordData(int theNumberOfVertices, int theLevel, int theTextureCoordSize) {
		...
		if (_myTextureCoordBuffers[theLevel] == null
				|| _myTextureCoords[theLevel].limit() / _myTextureCoordSize[theLevel] != _myNumberOfVertices) {
			if (_myTextureCoordBuffers[theLevel] == null) {
				_myTextureCoordBuffers[theLevel] = new CCBufferObject(
						_myNumberOfVertices * theTextureCoordSize * CCBufferUtil.SIZE_OF_FLOAT);
			} else {
				_myTextureCoordBuffers[theLevel].bufferData(
						_myNumberOfVertices * theTextureCoordSize * CCBufferUtil.SIZE_OF_FLOAT, null,
						CCUsageFrequency.DYNAMIC, CCUsageTYPE.DRAW);
			}
	...
	public void colors(CCShaderBuffer theShaderTexture, final int theX, final int theY, final int theWidth,
			final int theHeight) {
		colors(theShaderTexture, 0, theX, theY, theWidth, theHeight);
	}

	public void colors(CCShaderBuffer theShaderTexture, final int theID, final int theX, final int theY,
			final int theWidth, final int theHeight) {
		if (_myColorBuffer == null)
			_myColorBuffer = new CCBufferObject();
	...
	// _myNumberOfVertices = theWidth * theHeight;
	//
	// _myVertexBuffer.copyDataFromTexture(theShaderTexture, theID, theX, theY,
	// theWidth, theHeight);
	//
	// _myHasVertices = true;
	// _myHasUpdatedVertices = false;
	...
	public void enable(final CCGraphics g) {
		...
		for (int i = 0; i < _myHasTextureCoords.length; i++) {
			if (_myHasTextureCoords[i]) {
				...
				g.gl.glTexCoordPointer(_myTextureCoordSize[i], GL2.GL_FLOAT, 0, 0);
			}
		}
		if (_myHasColors) {
			...
			g.gl.glColorPointer(4, GL2.GL_FLOAT, 0, 0);
		}
		if (_myHasNormals) {
			...
			g.gl.glNormalPointer(GL2.GL_FLOAT, 0, 0);
		}
		if (_myHasVertices) {
			CCLog.info("YOYOYOY", _myVertices);
...
//				 _myVertexBuffer.bufferData();
			...
			g.gl.glVertexPointer(_myVertexSize, GL2.GL_FLOAT, 0, 0);
		}
		if (_myHasIndices) {
