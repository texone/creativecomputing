...
vec4 voronoi(vec2 x, vec3 mode )
{
    ...
    for( int i=-2; i<=2; i++ )
    {
        ...
        // triangular		
		vec2 d2 = vec2( max(abs(r.x)*0.866025+r.y*0.5,-r.y), step(0.0,0.5*abs(r.x)+0.866025*r.y)*(1.0+step(0.0,r.x)) );

		vec2 d = d0; //(d0 * mode.x + d1 * mode.y + d2 * mode.z) / (mode.x + mode.y + mode.z); 
