...
void main(){
	...
	gl_TexCoord[0] = vec4(myTexCoords,0,1);//vec4(mix(myTexCoords,myTexCoordsCenter,textureCenter) + myRandom.xy * myTextureRandom, 0, 1);
	gl_TexCoord[1] = vec4(myTexCoords,0,1);//vec4(mix(myTexCoords,myTexCoordsCenter,textureCenter) + myRandom.xy * saturationRandom, 0, 1);
